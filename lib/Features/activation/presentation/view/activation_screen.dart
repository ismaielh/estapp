...
class _ActivationScreenState extends State<ActivationScreen> {
  ...
  void initState() {
    ...
    if (subject != null) {
      ...
      // ابحث عن Subject الأم باستخدام LessonsCubit
      ...
      // ابحث عن Unit و Subject الأم باستخدام LessonsCubit
      final cubit = context.read<LessonsCubit>();
      if (cubit.state is LessonsLoaded) {
        final state = cubit.state as LessonsLoaded;
        final parentSubject = state.subjects.firstWhere(
  ...
  void processPayment() {
    ...
    try {
      if (selectedLessons.isNotEmpty) {
        final unitId = selectedUnitId ?? (unit?.id ?? _findUnitIdForLesson(lesson!));
      ...
      _showSuccessDialog(subject?.units.first.id ?? unit?.id);
    } catch (e) {
      developer.log('Payment processing error: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('error_processing_payment'.tr(args: [e.toString()]))),
      );
      setState(() {
        isProcessing = false;
      });
  ...
  void _showSuccessDialog(String? nextScreenId) {
    showDialog(
      context: context,
          ...
          TextButton(
            onPressed: () {
              ...
              if (nextScreenId != null) {
                context.push('/lessons/$nextScreenId');
              } else {
                context.pop();
              }
            },
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    if (selectedLessons.isNotEmpty || selectedUnitId != null || selectedSubjectId != null)
        ...
        Card(
          ...
          child: Padding(
            ...
            child: DropdownButton<String>(
              ...
              hint: Text('choose_subject'.tr(), style: Constants.activationSubTextStyle),
              dropdownColor: Constants.cardBackgroundColor,
              items: subject != null
                  ? [subject.id].map((String value) {
                      return DropdownMenuItem<String>(
                        value: value,
                        child: Text(subject.title, style: Constants.activationSubTextStyle),
                      );
                    }).toList()
                  : [],
        ...
        Card(
          ...
          child: Padding(
            ...
            child: DropdownButton<String>(
              ...
              hint: Text('choose_unit'.tr(), style: Constants.activationSubTextStyle),
              dropdownColor: Constants.cardBackgroundColor,
              items: units.map((Unit value) {
                return DropdownMenuItem<String>(
                  value: value.id,
                  child: Text(value.title, style: Constants.activationSubTextStyle),
                );
              }).toList(),
              onChanged: (String? newValue) {
                setState(() {
